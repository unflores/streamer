/*
  ...
  2. Respond to http requests and pass their json to listening socket clients
     curl -XPOST 'http://streamer.dev:8081/runs/5' -H "Content-Type: application/json" -d '{"id":"5","important_values": "stuff"}'
...
app.post(/runs\/(\d+)/, function(req, res){
  if(req.ip !== '127.0.0.1'){
    res.send("ERROR: Invalid Source.\n");
    return;
  }
  var news = req.body;
  news.room = 'run_' + req.params[0];
